...
CMP_CTX_set1_extraCerts() sets the stack of extraCerts that will be sent by the client with an initial/key update/certification requiest.

CMP_CTX_extraCerts_push1() adds the given certificate to the extraCerts stack, without consuming the pointer.
 
CMP_CTX_extraCerts_num() returns the number of certificates in the extraCerts stack.

CMP_CTX_caExtraCerts_num() returns the number of certificates that were received from the CA in an initialization/certification/key update response.

CMP_CTX_caExtraCerts_pop() pops and returns one certificate off the stack of extra certificates received from the CA.
