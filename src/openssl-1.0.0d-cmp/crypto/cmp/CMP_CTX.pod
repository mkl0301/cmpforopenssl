...
CMP_CTX_set1_subjectName() sets the subject name that will be used in the CertTemplate structure when requesting a new certificate. Note that if clCert or extCert are set, the name will be read from them and this will be ignored.

CMP_CTX_set1_extCert() sets the given external X509 certificate (to be used for in-band initialization), in the given CMP_CTX structure.
