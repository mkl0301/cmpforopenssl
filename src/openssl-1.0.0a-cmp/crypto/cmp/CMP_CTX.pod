...
CMP_CTX_set_compatibility() sets the given compatibility in the given CMP_CTX structure. This is needed since the CMP RFC can be interpreted in different ways so certain implementations may react different than others. Available Options are CMP_COMPAT_RFC for strict RFC conformace, CMP_COMPAT_CRYPTLIB for Cryptlib adaption, CMP_COMPAT_INSTA for Insta Certifier version < 3.3 adaption and CMP_COMPAT_INSTA_3_3 for Insta Certifier version => 3.3 adaption.
