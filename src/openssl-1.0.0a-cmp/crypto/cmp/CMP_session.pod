...
CMP_doCertificateRequestSeq() requests an additional certificate for an existing key from the CA. An initialized BIO and a populated CMP_CTX structure have to be given as arguments.

CMP_doPKIInfoReqSeq() requests information about the current status of a CA.
...
See CMP_CTX for examples on how to prepare the context for these functions.
