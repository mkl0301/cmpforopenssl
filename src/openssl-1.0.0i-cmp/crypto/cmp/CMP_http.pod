...
This is the API for creating a BIO for CMP (Certificate Management Protocol) over HTTP with OpenSSL.

CMP_new_http_bio() opens a OpenSSL BIO to communicate with the server.

CMP_new_http_bio_ex() opens a OpenSSL BIO to communicate with the server, with one additional argument to specify the source interface address.

CMP_PKIMESSAGE_http_perform() sends the given pkimessage msg through given connection, and returns the servers response in out.
