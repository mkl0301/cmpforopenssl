...
static int copyToOCSPRequest( CERT_INFO *ocspRequestInfoPtr,
	...
	/* Make sure that there's a CA cert hash present */
	...
	/* Add the cert information again as an ESSCertID extension to work 
	   around the problems inherent in OCSPv1 IDs.  Since this isn't a
	   critical extension, we just continue if there's a problem adding 
	   it */
