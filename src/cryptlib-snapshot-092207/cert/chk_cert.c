...
int checkPathConstraints( const CERT_INFO *subjectCertInfoPtr,
	...
	assert( isReadPtr( subjectCertInfoPtr, sizeof( CERT_INFO ) ) );
	assert( isReadPtr( issuerAttributes, sizeof( ATTRIBUTE_LIST ) ) && \
			issuerAttributes->fieldID == CRYPT_CERTINFO_PATHLENCONSTRAINT );
	assert( isWritePtr( errorLocus, sizeof( CRYPT_ATTRIBUTE_TYPE ) ) );
	assert( isWritePtr( errorType, sizeof( CRYPT_ERRTYPE_TYPE ) ) );
	...
	/* If the cert is self-signed (i.e. the cert is applying the constraint 
	   to itself) then a path length constraint of zero is valid.  Checking
	   only the subject cert info is safe because the calling code has
	   guaranteed that if the cert is self-signed, the issuer attributes are
	   the attributes from the subject cert */
