...
////////////////////////////////////////////////////////////
// The following code will segfault when built with gcc 4.x
// for x86-64 and -O3 due to an optimiser bug.  If this 
// occurs it's necessary to build with -O2 instead.
////////////////////////////////////////////////////////////
//	deleteDoubleListElement( dnListHead, theElement );
////////////////////////////////////////////////////////////
	if( theElement == *( dnListHead ) ) 
		{ 
		assert( theElement->prev == NULL );

		/* Special case for first item */ 
		*( dnListHead ) = theElement->next; 
		} 
	else 
		{ 
	...
	if( theElement->next != NULL ) 
		theElement->next->prev = theElement->prev; 
////////////////////////////////////////////////////////////
//	end of deleteDoubleListElement();
////////////////////////////////////////////////////////////
	...
	*stringTag = 0;
	...
	for( i = 0; certInfoOIDs[ i ].oid != NULL && \
		...
		if( certInfoOID->oid[ 4 ] == oid[ 4 ] && \
			...
			{
			*type = ( certInfoOID->type != CRYPT_ATTRIBUTE_NONE ) ?
					certInfoOID->type : i + DN_OID_OFFSET;
			break;
			}
		}
	if( i >= FAILSAFE_ARRAYSIZE( certInfoOIDs, DN_COMPONENT_INFO ) )
