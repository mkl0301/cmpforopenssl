...
static int getExtendedKeyUsageFlags( const ATTRIBUTE_LIST *attributes,
	...
	/* Most certs don't contain an extendedKeyUsage extension, so rather than
	   iterating through a long list of non-present attribute fields we 
	   perform an early-out check to see whether there's anything to do */
