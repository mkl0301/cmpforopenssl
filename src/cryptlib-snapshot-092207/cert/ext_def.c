...
static const ATTRIBUTE_INFO FAR_BSS extensionInfo[] = {
	...
	/* signingCertificate.  This is here even though it's a CMS attribute
	   ...
	   up the cert.identification information into bits and pieces and hashes
	   some while leaving others intact, there's no way to map what arrives 
	   at the responder back into a certificate without breaking the hash 
	   function.  To work around this, we include an ESSCertID in the request
	   that properly identifies the cert.being queried.  Since it's a 
	   limited-use version that only identifies the cert, we don't allow a 
	   full signingCertificate extension but only a single ESSCertID:
	...
	{ MKOID( "\x06\x0B\x2A\x86\x48\x86\xF7\x0D\x01\x09\x10\x02\x0C" ), CRYPT_CERTINFO_CMS_SIGNINGCERTIFICATE,
	  ...
	  BER_SEQUENCE, 0,
	  FL_MORE | FL_LEVEL_STANDARD | FL_VALID_OCSPREQ /*Per-entry*/, 0, 0, 0, NULL },
	{ NULL, 0,
	...
	{ NULL, CRYPT_CERTINFO_CMS_SIGNINGCERT_ESSCERTID,
	  ...
	  FIELDTYPE_BLOB, 0,
	  FL_SEQEND, 32, MAX_ATTRIBUTE_SIZE, 0, NULL },
	...
	/* ocspNonce:
	   ...
	   just dumps a raw blob (not even valid ASN.1 data) in here.  We can't 
	   do anything with this since we need at least something DER-encoded to 
	   be able to read it.  OpenSSL 0.9.7d and later used an OCTET STRING
	   so we use the same trick as we do for the certPolicy IA5String/
	   VisibleString duality where we define the field as if it were a 
	   CHOICE { INTEGER, OCTET STRING }, with the INTEGER first to make sure 
	   that we encode that preferentially.
	...
	{ MKOID( "\x06\x09\x2B\x06\x01\x05\x05\x07\x30\x01\x02" ), CRYPT_CERTINFO_OCSP_NONCE,
	  ...
	  FL_MORE | FL_LEVEL_OBLIVIOUS | FL_VALID_OCSPREQ | FL_VALID_OCSPRESP | FL_OPTIONAL, 1, 64, 0, NULL },
