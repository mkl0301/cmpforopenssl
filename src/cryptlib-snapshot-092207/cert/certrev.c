...
int readOcspRequestEntry( STREAM *stream, REVOCATION_INFO **listHeadPtr,
	...
	/* OCSPv1 uses a braindamaged cert.identification method that breaks the 
	   cert.information up into bits and hashes some while leaving others 
	   intact, making it impossible to identify the cert from it.  To try 
	   and fix this, if the request includes an ESSCertID we use that to
	   make it look like there was a proper ID present */
