...
typedef struct RI {
	/* Certificate ID information, either a serial number (for CRLs) or a
	   ...
	   this could also be a pre-encoded OCSP certID, which is treated as
	   an opaque blob of type CRYPT_ATTRIBUTE_NONE since it can't be used in
	   any useful way.  If we're using OCSP and an alternative ID is supplied
	   as an ESSCertID, we point to this value (inside the ESSCertID) in the 
	   altIdPtr field.  
...
typedef struct {
	/* The authenticator used for authenticating certificate issue and
	   revocation requests */
	BYTE pkiIssuePW[ 16 + 8 ], pkiRevPW[ 16 + 8 ];
	} CERT_PKIUSER_INFO;
...
typedef struct {
	...
	BYTE certHash[ KEYID_SIZE + 8 ];/* Cached cert hash */
