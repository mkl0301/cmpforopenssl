...
static int calculateKeyID( CONTEXT_INFO *contextInfoPtr )
	...
	if( publicKey->publicKeyInfo != NULL )
		...
		if( cryptAlgo == CRYPT_ALGO_RSA )
			...
			readSequence( &stream, NULL );
			status = readInteger( &stream, buffer, &length, 
								  CRYPT_MAX_PKCSIZE );
...
static int readEccSubjectPublicKey( STREAM *stream, CONTEXT_INFO *contextInfoPtr,
									int *actionFlags )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
	assert( isWritePtr( contextInfoPtr, sizeof( CONTEXT_INFO ) ) && \
...
static int readPublicKeyEccFunction( STREAM *stream, CONTEXT_INFO *contextInfoPtr,
									 const KEYFORMAT_TYPE formatType )
	{
	int actionFlags, status;
...
static int readEccPrivateKey( STREAM *stream, CONTEXT_INFO *contextInfoPtr )
	{
	PKC_INFO *dlpKey = contextInfoPtr->ctxPKC;
...
static int readPrivateKeyEccFunction( STREAM *stream, 
									  CONTEXT_INFO *contextInfoPtr,
									  const KEYFORMAT_TYPE formatType )
	{
	assert( isWritePtr( stream, sizeof( STREAM ) ) );
...
static int decodeDLValuesFunction( const BYTE *buffer, const int bufSize, 
	...
	switch( formatType )
		...
		case CRYPT_FORMAT_PGP:
			status = readBignumInteger16Ubits( &stream, *value1, 160 - 32,
											   bytesToBits( CRYPT_MAX_PKCSIZE ),
											   maxRange );
			if( cryptStatusOK( status ) )
				status = readBignumInteger16Ubits( &stream, *value2, 160 - 32,
												   bytesToBits( CRYPT_MAX_PKCSIZE ),
												   maxRange );
			break;
#endif /* USE_PGP */
		...
		case CRYPT_IFORMAT_SSH:
			status = extractBignum( *value1, buffer, 20, 
									bitsToBytes( 160 - 32 ), 20, maxRange );
			if( cryptStatusOK( status ) )
				status = extractBignum( *value2, buffer + 20, 20, 
										bitsToBytes( 160 - 32 ), 20, maxRange );
			break;
#endif /* USE_SSH */
