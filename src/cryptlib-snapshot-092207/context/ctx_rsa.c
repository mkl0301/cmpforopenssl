...
static int selfTest( void )
	...
	staticInitContext( &contextInfo, CONTEXT_PKC, getRSACapability(),
	...
	if( cryptStatusOK( status ) && \
		...
		contextInfo.flags |= CONTEXT_SIDECHANNELPROTECTION;
		status = capabilityInfoPtr->initKeyFunction( &contextInfo, NULL, 0 );
		if( cryptStatusOK( status ) )
...
static int decryptFn( CONTEXT_INFO *contextInfoPtr, BYTE *buffer, int noBytes )
	...
	const int length = bitsToBytes( pkcInfo->keySizeBits );
	int iterationCount = 0, offset, bnStatus = BN_STATUS, status;
