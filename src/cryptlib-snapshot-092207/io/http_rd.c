...
typedef struct {
	...
	int reqType;				/* Stream flag for this request type */
	} HTTP_REQUEST_INFO;

static const HTTP_REQUEST_INFO FAR_BSS httpReqInfo[] = {
	{ "GET", 3, STREAM_NFLAG_HTTPGET },
	{ "POST", 4, STREAM_NFLAG_HTTPPOST },
	{ NULL, 0, 0 }, { NULL, 0, 0 }
	};
...
static int readRequestHeader( STREAM *stream, char *lineBuffer,
	...
	for( i = 0; httpReqInfo[ i ].reqName != NULL && \
		...
		if( ( reqInfoPtr->reqType & stream->nFlags ) && \
	...
	if( i >= FAILSAFE_ARRAYSIZE( httpReqInfo, HTTP_REQUEST_INFO ) )
		retIntError();
	if( reqType <= 0 )
		{
	...
	bufPtr += offset;
	length -= offset;
	if( reqType == STREAM_NFLAG_HTTPGET )
		{
...
static int readResponseHeader( STREAM *stream, char *lineBuffer,
	...
	if( stream->nFlags & STREAM_NFLAG_HTTPGET )
		{
	...
	for( repeatCount = 0; repeatCount < FAILSAFE_ITERATIONS_SMALL; \
		...
		initHeaderInfo( &headerInfo, 5,
						...
						min( MAX_INTLENGTH, 8388608L ) : httpDataInfo->bufSize,
						*flags );
		status = readHeaderLines( stream, lineBuffer, lineBufSize,
								  &headerInfo );
		if( cryptStatusError( status ) )
...
static int readFunction( STREAM *stream, void *buffer, int length )
	...
	if( stream->nFlags & STREAM_NFLAG_ISSERVER )
		{
		assert( !( stream->nFlags & STREAM_NFLAG_HTTPGET ) || \
		...
		if( cryptStatusOK( status ) && \
			...
			assert( httpDataInfo->flags & HTTP_FLAG_DYNAMICBUFFER );
			...
			httpDataInfo->flags |= HTTP_FLAG_BUFFERRESIZED;
			}
		}
	if( cryptStatusError( status ) )
		return( status );
...
int setStreamLayerHTTP( STREAM *stream )
	...
	stream->nFlags |= STREAM_NFLAG_HTTPPOST;
