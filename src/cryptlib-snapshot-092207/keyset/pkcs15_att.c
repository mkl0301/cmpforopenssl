...
int readObjectAttributes( STREAM *stream, PKCS15_INFO *pkcs15infoPtr,
	...
	for( i = 0; \
		 ...
		 i++ );
	if( i >= FAILSAFE_ARRAYSIZE( allowedTypesTbl, ALLOWED_ATTRIBUTE_TYPES ) )
		retIntError();
	...
	switch( type )
		...
		case PKCS15_OBJECT_DATA:
			/* It's a data object, all of the attributes are optional.  If 
			   it's a cryptlib data object it'll be identified via the 
			   cryptlib OID */
			if( length <= 0 )
				break;
			if( peekTag( stream ) == BER_STRING_UTF8 )
			...
			if( cryptStatusOK( status ) && stell( stream ) < endPos && \
				...
				{
				status = readOID( stream, dataObjectOIDinfo, &value );
	...
	switch( type )
		...
		case PKCS15_OBJECT_DATA:
			...
			if( cryptStatusOK( status ) && \
				...
				{
				/* The config data and user index are SEQUENCEs of objects */
