...
/* Prototypes for functions in context/ctx_misc.c */
...
static int checkAnyTag( const int tag )
	{
...
static int readObjectHeader( STREAM *stream, int *length, const int minLength, 
	...
	if( tag == ANY_TAG )
		{
		/* We're prepared to accept any (valid) tag, make sure that it's
		   actually a potentially valid value */
		tagValue = checkAnyTag( tagValue );
		}
	if( tagValue != tag )
		return( sSetError( stream, CRYPT_ERROR_BADDATA ) );
...
int readBignumTag( STREAM *stream, void *bignum, const int minLength, 
				   const int maxLength, const void *maxRange, 
				   const int tag )
	{
	...
	if( !cryptStatusError( status ) )
		{
		status = extractBignum( bignum, buffer, length, 
								minLength, maxLength, maxRange );
		if( cryptStatusError( status ) )
...
int readOIDEx( STREAM *stream, const OID_INFO *oidSelection, 
	...
	for( oidEntry = 0; oidSelection[ oidEntry ].oid != NULL && \
		...
		if( sizeofOID( oidPtr ) == sizeofOID( WILDCARD_OID ) && \
			!memcmp( oidPtr, WILDCARD_OID, sizeofOID( WILDCARD_OID ) ) )
			{
			assert( oidSelection[ oidEntry + 1 ].oid == NULL );
			break;	/* The wildcard must be the last entry in the list */
			}
