...
/* Check a complex ASN.1 object.  In order to handle huge CRLs we can't use
   a fixed maximum iteration count but have to vary it based on the size
   of the input data.  Luckily this situation is relatively easy to check 
   for, it only occurs at a nesting level of 6 (when we find the CRL 
   entries), and we only have to enable it when the data length is more
   than 100K, since the default FAILSAFE_ITERATIONS_LARGE will handle 
...
static ASN1_STATE checkASN1( STREAM *stream, long length, const int isIndefinite,
	...
	{
	ASN1_ITEM item;
	const long maxIterationCount = ( level == 6 && length > 100000L ) ? \
