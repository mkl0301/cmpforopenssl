...
int base64checkHeader( const char *data, const int dataLength,
	...
	*startPos = 0;
	...
	/* Perform a quick check to weed out unencoded cert data, which is 
	   usually the case.  For very large objects (which can only be CRLs) we 
	   can get an overflow error trying to read a short length, so if the
	   length is suspiciously long we allow a long length.  We don't do this
