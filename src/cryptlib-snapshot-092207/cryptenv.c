...
static int instantiateCertChain( CONTENT_LIST *contentListItem, 
	...
	switch( messageValue )
		...
		default:
			assert( messageValue == CRYPT_ENVINFO_COMPRESSION || \
					...
					messageValue == CRYPT_ENVINFO_INTEGRITY || \
	...
	switch( messageValue )
		...
		case CRYPT_ENVINFO_DETACHEDSIGNATURE:
			/* If this isn't signed data or we haven't sorted out the content
			   ...
			   not.  We have to make an exception for PGP signed data because
			   the PGP format doesn't record whether a signature is a 
			   detached signature or not.  To resolve this, the lower-level
			   de-enveloping code takes a guess based on whether the user 
			   has manually added a hash for signed-data processing or not.
			   Because of this the detached-signature status can change from
			   (apparently-)false before adding the hash to (apparently-)
			   true after adding it, but there's not much that we can do to 
			   resolve this */
			if( envelopeInfoPtr->usage != ACTION_SIGN || \
		...
		case CRYPT_ENVINFO_SIGNATURE_RESULT:
			...
			/* If it's a MACd envelope then the signature result isn't held
			   in a content list as for the other signatures since the 
			   "signature" is just a MAC tag appended to the data.  The 
			   appropriate value to return here is a bit tricky since an
			   attacker could corrupt the MAC tag and force a less severe
			   error like CRYPT_ERROR_UNDERFLOW (by truncating the data).
			   However we can only get here once we've reached the finished 
			   state, which means that all of the data (including the MAC 
			   tag) has been successfully processed.  This means that any
			   persistent error state is regarded as the equivalent of a 
			   signature error */
			if( envelopeInfoPtr->usage == ACTION_MAC )
				{
			...
			if( sigInfo->iSigCheckKey == CRYPT_ERROR && \
				...
				instantiateCertChain( contentListItem, 
...
static int processGetAttributeS( ENVELOPE_INFO *envelopeInfoPtr,
	...
	assert( isWritePtr( envelopeInfoPtr, sizeof( ENVELOPE_INFO ) ) );
	assert( isWritePtr( messageDataPtr, sizeof( MESSAGE_DATA ) ) );
...
static int processSetAttribute( ENVELOPE_INFO *envelopeInfoPtr,
	...
	if( usage != ACTION_NONE )
		...
		switch( messageValue )
			...
			case CRYPT_ENVINFO_INTEGRITY:
				/* The integrity-protection flag can't be reset to a value 
				   of CRYPT_INTEGRITY_NONE once it's been set to a higher 
				   level.  If it could be reset then the caller could set 
				   non-MAC-compatible options by clearing the flag and then 
...
static int envelopePush( ENVELOPE_INFO *envelopeInfoPtr, const void *buffer,
...
static int deenvelopePush( ENVELOPE_INFO *envelopeInfoPtr, const void *buffer,
						   const int length, int *bytesCopied )
	{
	...
	*bytesCopied = 0;
	...
	if( envelopeInfoPtr->state == STATE_POSTDATA )
		...
		/* Process the postamble.  During this processing we can encounter
		   ...
		   one of these then we don't treat it as a standard error */
		status = envelopeInfoPtr->processPostambleFunction( envelopeInfoPtr );
		...
		assert( status == CRYPT_OK || status == OK_SPECIAL );
